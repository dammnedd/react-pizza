...
const CartItem = () => {
  const [countOfPizza, setCountOfPizza] = React.useState(1);

  const handleClickMinus = () => {
    if (countOfPizza !== 1) {
      setCountOfPizza((prev) => prev - 1);
    }
  };
  ...
  return (
    <div className={styles.cartItem}>
      ...
      <div className={styles.left}>
        <h1 className={styles.title}>Сырный цепленок</h1>
        <span className={styles.desc}>Толстое Тесто, 26см</span>
      </div>
      <div className={styles.count}>
        ...
        <img onClick={() => handleClickPlus()} className={styles.plus} src="/img/plusButton.svg" />
      </div>
